...
const WEBM_OUT = path.join(ROOT, 'polkastream-public-safe.webm');
const MP4_OUT = path.join(ROOT, 'polkastream-public-safe.mp4');
const APP_URL = 'http://127.0.0.1:3000/#console';
const MOCK_RPC_URL = 'https://mock.rpc.local/';
const DEBUG = process.env.DEBUG_RECORD === '1';
...
async function clickIfVisible(locator, waitAfter = 1200) {
...
async function connectWallet(page) {
  ...
  if (await metaMaskButton.isVisible().catch(() => false)) {
  ...
  if (to === CONTRACT) {
    ...
    switch (decoded.functionName) {
      ...
      case 'getSenderStreams': {
        ...
        if (DEBUG) {
          console.log('getSenderStreams', sender, ids.map((id) => id.toString()));
        }
      ...
      case 'getReceiverStreams': {
        ...
        if (DEBUG) {
          console.log('getReceiverStreams', receiver, ids.map((id) => id.toString()));
        }
...
async function run() {
  ...
  await page.mouse.move(520, 250, {steps: 18});
  await sleep(700);
  await page.mouse.move(920, 270, {steps: 24});
  await sleep(800);
  await page.mouse.move(1260, 260, {steps: 20});
  await sleep(4200);

  await page.getByRole('button', {name: 'Create Stream'}).first().click();
  await sleep(1400);
  await clickIfVisible(page.getByRole('button', {name: 'Immediate'}).first(), 900);
  ...
  const submitButton = page.locator('button.fx-rainbow-dark').filter({hasText: 'Create Stream'}).last();
  await submitButton.click();
  await page.getByText(/Stream created/i).waitFor({timeout: 8000}).catch(() => null);
  await sleep(2400);
  ...
  await clickIfVisible(page.getByRole('button', {name: 'Pause'}).first(), 2200);
  await clickIfVisible(page.getByRole('button', {name: 'Resume'}).first(), 2200);
  await clickIfVisible(page.getByRole('button', {name: 'Withdraw'}).first(), 2800);
  await clickIfVisible(page.getByRole('button', {name: /Retry Notify/i}).first(), 2600);

  await page.mouse.move(1270, 520, {steps: 20});
  await sleep(1000);
  await page.mouse.move(1330, 760, {steps: 18});
  await sleep(4200);
